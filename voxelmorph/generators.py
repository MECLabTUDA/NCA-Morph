...
def volgen(
    ...
    Parameters:
        ...
        rescale: Torchio rescaleIntensity between 0 and 1.
    """
    ...
    while True:
        ...
        load_params = dict(np_var=np_var, add_batch_axis=True, add_feat_axis=add_feat_axis,
                           pad_shape=pad_shape, resize_factor=resize_factor, rescale=rescale)
        ...
        if segs is True:
            ...
            load_params['rescale'] = None  # No rescaling for segs 
...
def scan_to_scan(vol_names, segs=None, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    ...
    vol_names_m = [x.replace('_0000.nii', '_0001.nii') for x in vol_names]
    segs_m = [x.replace('_0000.nii', '_0001.nii') for x in segs]
    gen_f = volgen(vol_names, segs=segs, batch_size=batch_size, **kwargs)
    gen_m = volgen(vol_names_m, segs=segs_m, batch_size=batch_size, **kwargs)

    while True:
        scan1, scan1_seg = next(gen_m)
        scan2, scan2_seg = next(gen_f)
        ...
        segs = [scan1_seg.astype(int), scan2_seg.astype(int)]
        # if not no_warp:
        #     outvols.append(zeros)

        # invols: [moving img, fixed img]
        # outvols: [fixed img, zeros] if not no_warp else (fixed img)
        # segs: [moving_seg, fixed seg]
        yield (invols, outvols, segs)
