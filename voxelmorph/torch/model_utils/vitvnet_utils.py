'''
TransMorph model from https://github.com/junyuchen245/ViT-V-Net_for_3D_Image_Registration_Pytorch/blob/main/ViT-V-Net/models.py
'''
# coding=utf-8
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        x = self.up(x)
        if x.size() != skip.size():
            x = torch.nn.functional.pad(input=x, pad=(0, skip.size(-1) - x.size(-1), 0, 0), mode='constant', value=0)
        if skip is not None:
